...
public class signupServ extends HttpServlet {
        ...
        if (isValidPassword(password)) {
            if (password.equals(repass)) {
                if (dao.checkAccountExist(username) != null) {
                    ...
                    response.sendRedirect("countPro");
                }
            } else {
                mess = "Repass must be the same pass!";
                request.setAttribute("mess", mess);
                request.getRequestDispatcher("login.jsp").forward(request, response);
            }
        } else {
            mess = "Password must contain characters and number!";
            request.setAttribute("mess", mess);
            request.getRequestDispatcher("login.jsp").forward(request, response);
        }
    ...
    public static boolean isValidPassword(String password) {
        // Kiểm tra xem mật khẩu có ít nhất một chữ cái, một số và một kí tự hay không
